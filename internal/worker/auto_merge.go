...
func RunAutoMerge(h Host, sessionID string) {
	...
	for attempt := 1; attempt <= maxAutoMergePollAttempts; attempt++ {
		...
		action := CheckAndAddressComments(h, sessionID, sess, attempt)
		switch action {
		...
		action = CheckReviewApproval(h, sessionID, sess, attempt)
		switch action {
		case MergeActionContinue:
			continue
		case MergeActionStop:
		...
		action = CheckCIAndMerge(h, sessionID, sess, attempt)
		switch action {
		case MergeActionContinue:
			continue
		case MergeActionStop:
...
func CheckAndAddressComments(h Host, sessionID string, sess *config.Session, attempt int) MergeAction {
	log := h.Logger().With("sessionID", sessionID)

	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()

	results, err := h.GitService().GetBatchPRStatesWithComments(ctx, sess.RepoPath, []string{sess.Branch})
	if err != nil {
	...
	if result.CommentCount > sess.PRCommentsAddressedCount {
		...
		if addressComments(h, sessionID, sess) {
...
func addressComments(h Host, sessionID string, sess *config.Session) bool {
	log := h.Logger().With("sessionID", sessionID)

	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()

	comments, err := h.GitService().FetchPRReviewComments(ctx, sess.RepoPath, sess.Branch)
	if err != nil {
...
func CheckReviewApproval(h Host, sessionID string, sess *config.Session, attempt int) MergeAction {
	log := h.Logger().With("sessionID", sessionID)

	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()

	reviewDecision, err := h.GitService().CheckPRReviewDecision(ctx, sess.RepoPath, sess.Branch)
	if err != nil {
...
func CheckCIAndMerge(h Host, sessionID string, sess *config.Session, attempt int) MergeAction {
	log := h.Logger().With("sessionID", sessionID)

	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()

	ciStatus, err := h.GitService().CheckPRChecks(ctx, sess.RepoPath, sess.Branch)
	if err != nil {
	...
	case git.CIStatusPassing, git.CIStatusNone:
		...
		return DoMerge(h, sessionID, sess)
...
func DoMerge(h Host, sessionID string, sess *config.Session) MergeAction {
	log := h.Logger().With("sessionID", sessionID)
	...
	err := h.GitService().MergePR(ctx, sess.RepoPath, sess.Branch, false, h.MergeMethod())
	if err != nil {
	...
	if h.Config().GetAutoCleanupMerged() {
		if err := h.CleanupSession(context.Background(), sessionID); err != nil {
