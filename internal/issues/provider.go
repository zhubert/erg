...
type ProviderSectionChecker interface {
	...
	// (Asana) or workflow state (Linear). The name is matched case-insensitively.
...
// named section or workflow state. This is optional; only providers that support
// board sections (Asana) or workflow states (Linear) need to implement it.
type ProviderSectionMover interface {
	// MoveToSection moves the issue/task to the named section (Asana) or
	// workflow state (Linear). The name is matched case-insensitively.
	// Returns an error if the section/state is not found or the API call fails.
