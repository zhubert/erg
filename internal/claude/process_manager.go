...
// containerStartupTimeoutDuration returns the effective container startup timeout,
// using the config override if set, otherwise the package-level default.
func (pm *ProcessManager) containerStartupTimeoutDuration() time.Duration {
	if pm.config.ContainerStartupTimeout > 0 {
		return pm.config.ContainerStartupTimeout
	}
...
func (pm *ProcessManager) handleExit(err error) {
	...
	if wasContainerTimeout {
		startupTimeout := pm.containerStartupTimeoutDuration()
		pm.log.Error("container startup timed out", "timeout", startupTimeout)
...
// process if it doesn't produce output within the configured startup timeout.
...
func (pm *ProcessManager) containerStartupWatchdog() {
	startupTimeout := pm.containerStartupTimeoutDuration()
	pm.log.Debug("container startup watchdog started", "timeout", startupTimeout)
	...
	for {
		...
		case <-ticker.C:
			...
			remaining := startupTimeout - elapsed
